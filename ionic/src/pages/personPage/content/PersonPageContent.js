...
import { IonItem, IonContent, IonList, IonPage, IonSpinner, IonItemSliding, IonItemOption, IonItemOptions, IonAlert } from '@ionic/react';
...
const PersonPageContent = () => {
    ...
    const { loading, myAnouncement, alert, setAlert, alertMessage, setIdDeleteAnnouncement , idDeleteAnnouncement, onDelete } = personPageContextData;
    return (
        <>
        <IonPage>
            <Header linkTo={HOME_PAGE}/>
            <IonContent >
                <div className="personPageAnouncementTitle">My anouncement <span>&#8595;</span></div>
                {loading ? 
                    <IonSpinner/> :
                    <IonList>
                        {myAnouncement.map(announcement=> {
                            return <IonItemSliding key={announcement.id}>   
                                        <IonItem>
